...
**How to apply:** When changing a default, list models and run a minimal server-only generation or embedding with the project’s direct Google API access. Verify the returned embedding dimension before creating or reusing a vector collection. Do not rely on naming conventions or provider-agnostic model lists. Use `title` only for `RETRIEVAL_DOCUMENT`; Google rejects it for `RETRIEVAL_QUERY`.